...
/*--------------------------------------------
/*--------------------------------------------
/* THIS CODING WORKING GOOD 
function check(){
  let dataElement = document.querySelector('input').value;
  let x ;
  for (i=0 ; i< cities.length; i++){
    if ((cities[i].city.search(dataElement) != -1) || (cities[i].state.search(dataElement) != -1)){
...
function marking(str,regx){
  return str.replace(regx,`<mark>$&</mark>`)

};

function payAttention(num){
...
citiesForm.addEventListener('input',(e)=> {
  ...
  const html = newMatches.map(element => {
    return `<li>${marking(element.city,regex)},${element.state},${payAttention(element.population)}</li>`
  });
   console.log(html);
